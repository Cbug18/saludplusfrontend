...
const Pacientesagendados= () => {
    ...
    useEffect(() => {
        ...
        const fetchAppointments = async () => {
            try {
                const response = await axios.get('https://saludplusweb-production.up.railway.app/api/getpatients');
                ...
                <div className="overflow-auto rounded-lg shadow hidden md:block">
                    <table className="w-3/5 mx-auto">
                        <thead className="bg-gray-50 border-b-2 border-gray-200">
                            <tr>
                                <th className="w-24 p-3 text-sm font-semibold tracking-wide text-left">Fecha De Nacimiento</th>
                                <th className="p-3 text-sm font-semibold tracking-wide text-left">Pacientes</th>
                                <th className="p-3 text-sm font-semibold tracking-wide text-left">Sintomas</th>
                                <th className="p-3 text-sm font-semibold tracking-wide text-left">Alergias</th>
                                <th className="p-3 text-sm font-semibold tracking-wide text-left">Condiciones Preexistentes</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {appointments.map((appointment) => (
                                <tr key={appointment._id} className="bg-white">
                                    <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{new Date(appointment.birthdate).toLocaleDateString()}</td>
                                    <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{appointment.firstNames}</td>
                                    <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{appointment.symptoms}</td>
                                    <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{appointment.allergies}</td>
                                    <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{appointment.preExistingConditions}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                    ...
                    {appointments.map((appointment) => (
                        <div key={appointment._id} className="bg-white space-y-3 p-4 rounded-lg shadow">
                            <div className="flex items-center space-x-2 text-sm">
                                <div className="text-gray-500">{new Date(appointment.dateOfAppointment).toLocaleDateString()}</div>
                                <div className="text-sm text-gray-700">{appointment.firstNames}</div>
                                <div className="text-sm text-gray-700">{appointment.symptoms}</div>
                                <div className="text-sm text-gray-700">{appointment.allergies}</div>
                                <div className="text-sm text-gray-700">{appointment.preExistingConditions}</div>
                            </div>
                        </div>
                    ))}
                </div>
